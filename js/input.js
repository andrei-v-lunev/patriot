...
(function () {
  ...
  function coopKeyboard() {
    return !!(window.PScreens && PScreens.modeId && PScreens.modeId() === "COOP");
  }
  ...
  function pollKeyboard() {
    ...
    var w = kbdAxis("KeyW", "KeyS", "KeyA", "KeyD");
    var arrows = kbdAxis("ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight");
    var mx = w.x, md = w.d;
    if (!coop) {
    ...
    edge(0, "throw", !!(keys.KeyJ || keys.KeyZ));       // «УДАР» strike, alt Z
    edge(0, "jump", !!(keys.KeyK || keys.KeyX || keys.Space)); // «ПРЫЖОК» jump, alt X (Space kept as legacy alias)
    edge(0, "grip", !!(keys.KeyL || keys.KeyC));        // «ЗАХВАТ / БРОСОК» grip/throw, alt C
    edge(0, "special", !!(keys.KeyU || keys.KeyV));      // «СПЕЦПРИЁМ», alt V
    edge(0, "tag", !!(keys.KeyI || keys.KeyB));          // «СМЕНА» tag, alt B
    edge(0, "ukemi", !!(keys.KeyO || keys.KeyN));        // «УКЭМИ», alt N
    ...
    edge(1, "throw", !!(keys.Numpad1 || keys.Period));      // strike, alt `.`
    edge(1, "jump", !!(keys.Numpad2 || keys.Slash));        // jump, alt `/`
    edge(1, "grip", !!(keys.Numpad3 || keys.ShiftRight));   // grip/throw, alt Right Shift
    edge(1, "special", !!(keys.Numpad5 || keys.Semicolon)); // alt `;`
    edge(1, "tag", !!(keys.Numpad6 || keys.Quote));         // alt `'`
    edge(1, "ukemi", !!(keys.Numpad0 || keys.Comma));       // alt `,`
