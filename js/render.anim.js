/* Sprite-sheet atlas loader + deterministic frame picker.
   ...
   tick counter (60Hz) — never wall-clock. */
(function () {
  var atlas = null;
  var sheets = {};
  var loading = false;
  ...
  /* Looping frame from the global sim tick (60Hz). */
  function frame(id, tick) {
    if (!has(id)) return null;
    var m = atlas[id];
    var fps = m.fps || 8;
    var n = m.frames || 1;
    var step = Math.floor(((tick | 0) * fps) / 60) % n;
    if (step < 0) step = 0;
    return pack(id, step);
  }
  ...
  function frameOnce(id, ticksSinceStart) {
    ...
    var fps = m.fps || 8;
    var n = m.frames || 1;
    var step = Math.floor(((ticksSinceStart | 0) * fps) / 60);
    if (step < 0) step = 0;
    if (step >= n) step = n - 1;
    return pack(id, step);
  }
